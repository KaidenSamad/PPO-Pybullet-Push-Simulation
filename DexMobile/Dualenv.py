...
class Dualenv(gym.Env):
    ...
    def reset(self, **kwargs):
        ...
        self.grasp = "platform"  # open-hand push task (overrides CSV topology)
        self.gl_error = 0    # disable precision slowdowns for push task
        self.near_error = 0  # arm moves at full speed the whole time
    ...
    def step(self, action):
        ...
        if self.grasp == "platform":
            # target is 15cm past the bottle — arm always tries to push through
            self.p_new = np.array([obPos[0] + 0.15, obPos[1], obPos[2] + 0.03])
            self.stage = 1  # arm keeps moving throughout for push task
        elif self.inPos(self.gl_error):
            self.stage = 2
            self.in_pos = 1
        ...
        if self.stage == 1:
            ...
            if self.grasp == "platform":
                realAction = [dx, 0, 0, 0, 0, 0, 0, 0]  # unlocked for screenshot — revert after
            else:
    ...
    def _termination(self, action):
        ...
        if self.grasp == "platform":
            # for push task: only fail if bottle falls off the table
            obPos, _ = p.getBasePositionAndOrientation(self.object)
            if obPos[2] < -0.1:
                self._observation = self.getExtendedObservation()
                print(self.task_id, " Terminated: bottle fell off table")
                self.fail_reason = "out of range"
                self.out_of_range = 1
                time.sleep(1)
                return True
            # success: bottle pushed off the table (z drops below table surface)
            if obPos[2] < 0.05:
                self._graspSuccess = 1
                self._observation = self.getExtendedObservation()
                print(self.task_id, ": Push Succeeded!! Bottle off table!")
                self.fail_reason = "success"
                time.sleep(1)
                return True
        else:
    ...
    def _reward(self):
        ...
        if self.grasp == "platform":
            self.stage = 1  # always stage 1 for push task
        elif self.inPos(self.gl_error):
            self.stage = 2
            self.in_pos = 1
        elif self.in_pos == 1 and self.inGrasp():
    ...
    def reward_s1(self):
        ...
        if self.grasp == "platform":
            ...
            obPos, _ = p.getBasePositionAndOrientation(self.object)
            # reward arm moving in x toward bottle (fires immediately, before contact)
            x_arm_progress = max(0, p_hand[0] - 0.65) * 200
            # larger reward: bottle actually moving in +x (fires after contact)
            x_bottle_progress = max(0, obPos[0] - self.p_obj[0]) * 500
            # small reward: touching bottle
            contact_reward = contact_num * 5
            # y/z penalty: reduced weight so it doesn't stop x movement
            y_deviation = abs(p_hand[1] - obPos[1])
            z_deviation = abs(p_hand[2] - (obPos[2] + 0.03))
            alignment_penalty = (y_deviation + z_deviation) * 30
            reward = x_arm_progress + x_bottle_progress + contact_reward - alignment_penalty
            return reward

        # original reward for all other grasp types
        dist = self.hp.distant(p_hand, self.p_new)
        reward_move = 1 / (dist + 1)
        ...
        reward_direct = self.hp.calculate_direction(initial_direction, current_direction)
        contact_points = p.getContactPoints(self._dual.dualUid, self.object)
        contact_num = len(contact_points)
    ...
    def pickup(self):
        ...
        if self.grasp is None:
            return False
        if self.grasp == "platform":
            return False  # push task does not use pickup/lift
        if self.grasp == "inSiAd2":
