...
class Helper:
    ...
    def __init__(self):
        ...
        self.dualEndEffectorIndex = 90
        self.endEffectorPos = np.array([0.65, -0.455, 0.25])
        ...
        # all revolute joints
        self.r_joint_id = [3, 4, 5, 6, 13, 14, 19, 20, 21, 22, 23, 24, 32, 34, 35, 36, 40, 41, 42, 43, 45, 46, 47, 52,
                           53, 54, 55, 57, 58, 59, 60, 65, 66, 67, 68, 69, 70, 78, 80,
                           81, 82, 86, 87, 88, 89, 91, 92, 93, 98, 99, 100, 101, 103, 104, 105, 106]
    ...
    ########################################## robot rest pos ################################################
        ...
        # left arm
        self.left_arm_initial_pos = [1.9841637611389125, -2.5794131755828786, 2.3809964656829763,
                                     1.322776317596432,
                                     2.116441249847419, 0]
        ...
        # right arm
        self.right_arm_initial_pos = [-0.0023891079478327083, -2.8163905456766303, 1.9667964054643494,
    ...
    def select_task(self):
        ...
        if len(df) > 50000 and df['tid'].nunique() == 27:
            # Calculate success rate for each tid
            success_rates = df.groupby('tid')['s'].sum() / df.groupby('tid')['s'].count()
            # Sort success rates and find the 10 tids with the least success rate
            least_success_rate_tids = success_rates.nsmallest(10).index.tolist()
            # Randomly select one from the 10 tids with the least success rate
            return random.choice(least_success_rate_tids)
        else:
            return random.randint(0, 26)
